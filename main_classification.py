...
def PrepareDataset_Classification(speed_matrix,
                                  BATCH_SIZE=40,
                                  seq_len=10,
                                  pred_len=1,
                                  train_propotion=0.7,
    ...
    for i in range(time_len - seq_len - pred_len):
        speed_sequences_temp = speed_matrix.iloc[i:i + seq_len].values
        speed_sequences.append(speed_sequences_temp)
        mean = np.mean(speed_sequences_temp)
        speed_labels_temp = speed_matrix.iloc[i + seq_len:i + seq_len + pred_len].values
        temp = []
        for value in speed_labels_temp[0]:
            if value > mean:
                temp.append(1)
            else:
                temp.append(0)
        speed_labels.append(temp)
    speed_sequences, speed_labels = np.asarray(speed_sequences), np.asarray(speed_labels)
    speed_labels = np.expand_dims(speed_labels, axis=1)
    print(f"{speed_sequences.shape = }")
    print(f"{speed_labels.shape = }")

    speed_sequences /= max_speed
...
def Train_Model(model, train_dataloader, valid_dataloader, num_epochs=10, patience=10, min_delta=0.00001,
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_dataloader:
            ...
            if output_last:
                loss_train = loss_BCE(torch.squeeze(logits), torch.squeeze(labels))
            else:
                full_labels = torch.cat((inputs[:, 1:, :], labels), dim=1)
                loss_train = loss_BCE(logits, full_labels)
            ...
            if output_last:
                ...
                loss_valid = loss_BCE(outputs_val, full_labels_val)
...
if __name__ == "__main__":
    ...
    sample_data = create_sample_data(entries=10000)

    train_dataloader, valid_dataloader, test_dataloader, max_speed, X_mean = PrepareDataset_Classification(speed_matrix,
                                                                                                           BATCH_SIZE=64,
                                                                                                           masking=True,
                                                                                                           seq_len=10)
